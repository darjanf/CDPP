...
import Plutus.V2.Ledger.Api       ( OutputDatum(..), TxOut(txOutAddress, txOutValue),
                                   ...
                                   ValidatorHash (ValidatorHash), txOutValue, Validator, PubKeyHash(PubKeyHash))
...
import Plutus.V2.Ledger.Contexts  ( txSignedBy, scriptOutputsAt, ownCurrencySymbol, txInfoOutputs)
import PlutusTx                   ( compile, unstableMakeIsData,
                                   liftCode, applyCode, makeLift, CompiledCode, toBuiltinData)
import PlutusTx.Builtins.Class    (stringToBuiltinString)
import PlutusTx.Builtins.Internal (unsafeDataAsB)
import PlutusTx.Prelude           ( Bool(False), Integer, Maybe(..), (.), negate, traceError,
                                   (&&), traceIfFalse, ($), Ord((<), (>), (>=)), Eq((==)), divide,
                                   MultiplicativeSemigroup((*)), decodeUtf8, head, fst, snd, filter)
import qualified Prelude          ( Show (show), IO)
import           PriceFeedOracle  ( parseOracleDatum)
import           Collateral       ( CollateralDatum (..), stablecoinTokenName, parseCollateralDatum)
import qualified Collateral       as Collateral
import qualified PriceFeedOracle  as Oracle
import           Utilities        (wrapPolicy, writeCodeToFile)
import           Serialization    (writePolicyToFile, writeDataToFile)
import qualified UtilitiesPPP     as UtilitiesPPP
...
--                        list     -> traceError $ decodeUtf8 $ unsafeDataAsB $ toBuiltinData $ snd $ head list

    collateralOutput' :: (OutputDatum, Value)
    collateralOutput' = case outputEntry of
        [e] -> (txOutDatum e, txOutValue e)
        []  -> traceError "no collateral output found!"
        _   -> traceError "more than one collateral output found!"
        where
            outputEntry :: [TxOut]
            outputEntry = filter (\x -> checkValidatorHash x) (txInfoOutputs info)

            validatorHash :: ValidatorHash
            validatorHash = mpCollateralValidator mp

            checkValidatorHash :: TxOut -> Bool
            checkValidatorHash txOut = case toValidatorHash (txOutAddress txOut) of
                Just v  -> traceIfFalse (decodeUtf8 $ unsafeDataAsB $ toBuiltinData $ v) (v == validatorHash)
                _       -> False
    ...
    checkDatum = case collateralOutputDatum of
        Nothing -> traceIfFalse "checkDatum return Nothing" False
        Just d  -> traceIfFalse "currencySymbol not equal" (colMintingPolicyId d  == ownCurrencySymbol ctx) &&
                   traceIfFalse "amount not equal" (colStablecoinAmount d == mintedAmount) && 
                   traceIfFalse "signedBy not equal" (txSignedBy info (colOwner d))
...
saveCollateralDatumAlice :: Prelude.IO ()
saveCollateralDatumAlice = writeDataToFile "./data/collateralDatumAlice.json" CollateralDatum { 
    colMintingPolicyId  = UtilitiesPPP.currencySymbol $ policy mintingParam
    , colOwner = "c4034310db8742a0c48539c26aa9890d10961925ffcde9de450581cc"
    , colStablecoinAmount = 2
    }

saveCollateralDatumBob :: Prelude.IO ()
saveCollateralDatumBob = writeDataToFile "./data/collateralDatumBob.json" CollateralDatum { 
    colMintingPolicyId  = UtilitiesPPP.currencySymbol $ policy mintingParam
    , colOwner = "d70540296a8155451197472f28ab6c5c4e9830d5c2f1b615307ed10b"
    , colStablecoinAmount = 2
    }

mintingParam :: MintParams
mintingParam = MintParams
    { mpOracleValidator = UtilitiesPPP.validatorHash' $ Oracle.validator Oracle.param
    , mpCollateralValidator = UtilitiesPPP.validatorHash' Collateral.validator
    , mpCollateralMinPercent = 150
    }

saveMintingPolicy :: Prelude.IO ()
saveMintingPolicy =  writePolicyToFile "./validators/minting.plutus" $ policy mintingParam

saveMintRedeemer:: Prelude.IO ()
saveMintRedeemer = writeDataToFile "./data/mintRedeemer.json" (Mint::MintRedeemer)

saveBurnRedeemer :: Prelude.IO ()
saveBurnRedeemer = writeDataToFile "./data/burnRedeemer.json" (Burn::MintRedeemer)

saveLiquidateRedeemer :: Prelude.IO ()
saveLiquidateRedeemer = writeDataToFile "./data/liquidateRedeemer.json" (Liquidate::MintRedeemer)
...
saveAll = do
            ...
            saveMintRedeemer
            saveBurnRedeemer
            saveLiquidateRedeemer
            saveCollateralDatumAlice
            saveCollateralDatumBob